...
(function() {
  ...
  $(window).on('code-editor-init', function(e, target) {
    $.require.load(
      ...
      function() {
        ...
        var mode = target.attr('data-mode');
        var width = target.attr('data-width') || 0;
   ...
   * Phone Field
   */
  $(window).on('phone-field-init', function(e, trigger) {
    $.require(
      [
      ...
      function() {
        var country = $(trigger).data('country') || 'us';
        var name = $(trigger).attr('name');
        var target = $('<input type="hidden" />').attr('name', name);

        //remove name and add target
        $(trigger).removeAttr('name').after(target);
        //start the widget
        var input = intlTelInput(trigger, {
          separateDialCode: true,
          nationalMode: false,
          initialCountry: country.toLowerCase()
        });

        var activeCode = '+' + input.getSelectedCountryData().dialCode;

        $(trigger).on('countrychange', function() {
          var value = $(target).val();
          var code = '+' + input.getSelectedCountryData().dialCode;
          if (value.indexOf(activeCode) === 0) {
            $(target).val(code + ' ' + value.substr(activeCode.length));
          }
          ...
          setTimeout(function() {
            ...
            //if the number does not start with plus
            if (value.length) {
              if (value.indexOf('+') !== 0) {
                $(target).val((activeCode + ' ' + value).replace(/\s+/g, ' '));
              } else {
                $(target).val(value.replace(/\s+/g, ' '));
              }
            } else {
              $(target).val('');
            }
